...
ALLData['species'] = np.select(
        [ALLData['species'].eq('Adelie'), ALLData['species'].eq('Gentoo'),ALLData['species'].eq('Chinstrap')], ['100', '010','001'],
    )
...
###############calculate weights
def weight_generat(input,output):
    temp=input*output
    for i in range(temp):
        w = random.random()
        hidden.append(w)
...
def ActivationFunction(net):
    for Type in range(len(g.function)):
        if (g.function[Type] == 'Sigmoid function'):
            f = float(1.0 / (1.0 + float(np.exp(-net))))


            break
        elif (g.function[Type] == 'Hyperbolic Tangent'):
            net= float(net)
            tempo= float(1.0 - float(np.exp(-net)))
            tempo2=float(1.0 + float(np.exp(-net)))
            ...
            for c in reversed(range(len(neuron))):
                ...
                for neuron in reversed(layer):
                    ...
                    for num,w in zip(neuron,range(len(neuron))):
                        ...
                        new.append(fun[(nn+1)+w])
                        error+=list(neuron)[c]*errors[m]
                        m=m+1
                        break
                error=error*fun[(nn+1)+w]*(1-fun[(nn+1)+w])
                m=m-len(layer)
                errors.append(error)
                row2=new
                nn +=1
            ...
            if (a == 0):
                ...
                for b in range(len(weights[a])):
                    ...
                    for c, e in zip(range(len(weights[a][b])), range(lenth)):
                        if(g.flages[0]==1and c==len(weights[a][b])-1):
